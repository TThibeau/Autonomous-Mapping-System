...
namespace rolling_occupancy_grid_ns {
    ...
    bool RollingOccupancyGrid::UpdateRobotPosition(const Eigen::Vector3d &robot_position) {
        ...
        for (int i = 0; i < dimension_; i++) {
            robot_grid_sub(i) =
                    diff(i) > 0 ? static_cast<int>(diff(i) / (rollover_step_size_(i) * resolution_(i))) : -1;
        }
        ...
        for (int i = 0; i < dimension_; i++) {
            rollover_step(i) =
                    std::abs(sub_diff(i)) > 0 ? rollover_step_size_(i) * ((sub_diff(i) > 0) ? 1 : -1) *
                                                std::abs(sub_diff(i)) : 0;
        }
...
//        ROS_INFO_STREAM("points size " << cloud->points.size()); // It seems this is always called for an empty cloud?
...
//                            ROS_INFO("SetCellValue: FREE - RollingOccupancyGrid::RayTrace");
                        }
                    }
                }
            }
    ...
    void RollingOccupancyGrid::GetFrontier(pcl::PointCloud<pcl::PointXYZI>::Ptr &free_cloud,
                                           pcl::PointCloud<pcl::PointXYZI>::Ptr &occupied_cloud,
                                           pcl::PointCloud<pcl::PointXYZI>::Ptr &unknown_cloud,
                                           pcl::PointCloud<pcl::PointXYZI>::Ptr &frontier_cloud,
        ...
        for (int ind = 0; ind < cell_num; ind++) {
            ...
            // ---->
            // Frontier cells are by definition:
            // Unknown
            // Neigbor to at least 1 FREE cell
...
//        ROS_INFO_STREAM("In GetFrontier - cell_num: " << cell_num << ", in_range: " << in_range_cnt << ", unknown: "
//                                                      << unknown_cnt << ", free: " << free_cnt << ", occupied: "
//                                                      << occupied_cnt);
    }
