...
int SensorCoveragePlanner3D::UpdateViewPoints()
{
  ...
  pd_.collision_cloud_->cloud_ = pd_.planning_env_->GetCollisionCloud(); // Does this method exist? Should it be UpdateCollisionCloud() instead?
  ...
  // For visualization
  pd_.collision_cloud_->Publish(); // Shows the pointcloud of obstacles
  pd_.viewpoint_manager_->GetCollisionViewPointVisCloud(pd_.viewpoint_in_collision_cloud_->cloud_);
  pd_.viewpoint_in_collision_cloud_->Publish(); // Shows the viewpoints that are considered to collide with obstacles
...
void SensorCoveragePlanner3D::UpdateCoveredAreas(int& uncovered_point_num, int& uncovered_frontier_point_num)
{
  ...
  pd_.planning_env_->PublishFreeOccupiedUnknownCloud();
}
...
void SensorCoveragePlanner3D::UpdateGlobalRepresentation()
{
  ...
  pd_.planning_env_->UpdateKeyposeCloud<PlannerCloudPointType>(pd_.keypose_cloud_->cloud_); // This includes updating the frontiers too
...
void SensorCoveragePlanner3D::execute(const ros::TimerEvent&)
{
  ...
  if (keypose_cloud_update_) // Happens after 5 registered scan callbacks
  {
    keypose_cloud_update_ = false;
